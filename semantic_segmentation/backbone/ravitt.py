    ...
    if self.grad_checkpointing and not torch.jit.is_scripting():
        ...
        for i, block in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                xp = x[:, 1:, :].permute(0, 2, 1).reshape(B, -1, self.patch_size, self.patch_size)
...
class RaViTT(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """

    def __init__(self, model_path='deit_tiny_patch16_224', t=1.0, img_size=224, patch_size=16, in_chans=3, num_classes=80, use_checkpoint=False,
        ...
        self.model = create_model(model_path, img_size=img_size, patch_size=patch_size, in_chans=in_chans)
        ...
        if patch_size == 16:
            self.fpn1 = nn.Sequential(
                ...
                nn.SyncBatchNorm(self.model.embed_dim),
                nn.GELU(),
                nn.ConvTranspose2d(self.model.embed_dim, self.model.embed_dim, kernel_size=2, stride=2),
            )
